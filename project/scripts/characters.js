...
characterElements.forEach(character => {
    character.addEventListener('click', function () {
        const characterKey = character.getAttribute('data-character');
        const characterData = characters[characterKey];

        // Check if the clicked character is already active
        const isActive = character.classList.contains('active');

        // Close all active dropdowns and reset styles
        characterElements.forEach(c => {
            c.classList.remove('active');
            const existingDetails = c.querySelector('.character-details');
            if (existingDetails) existingDetails.remove();
            const image = c.querySelector('img');
            const caption = c.querySelector('h3');
            if (image) image.style.display = 'block';
            if (caption) caption.style.display = 'block';
        });

        // If the clicked character is not active, show the details
        if (!isActive) {
            const imgElement = character.querySelector('img');
            const captionElement = character.querySelector('h3');
            if (imgElement) imgElement.style.display = 'none';
            if (captionElement) captionElement.style.display = 'none';

            const detailsContent = document.createElement('div');
            detailsContent.classList.add('character-details');
            detailsContent.innerHTML = `
                <img src="${characterData.image}" alt="${characterData.name}">
                <h3>${characterData.name}</h3>
                <p>${characterData.description}</p>
            `;
            character.appendChild(detailsContent);
